...
impl Parsable for AuthData {
    ...
    fn parser () -> Self::Parser {
        map(take(AUTHDATA_SIZE, stdp::Byte), |authdata| AuthData(authdata.try_into().unwrap_or(Box::new([0;AUTHDATA_SIZE]))))
    }
}
...
pub struct LogLineParser {
    parser: OnceLock<<LogLine as Parsable>::Parser>,
}
impl LogLineParser {
    pub fn new() -> Self {
        Self { parser: OnceLock::new() }
    }
